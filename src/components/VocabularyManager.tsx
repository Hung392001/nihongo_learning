  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        alert(`Successfully imported ${successCount} of ${importedData.length} words!`);
      } catch (error) {
                ...
                <>
                  ...
                  <div className="bulk-actions">
                    <label className="select-all-label">
                      <input
                        type="checkbox"
                        checked={selectedIds.size === processedVocabulary.length && processedVocabulary.length > 0}
            ...
            {processedVocabulary.length === 0 ? (
              ...
              <div className="vocab-items">
                {processedVocabulary.map((item) => (
                  <div key={item.id} className={`vocab-item ${selectedIds.has(item.id) ? 'selected' : ''}`}>
