...
const config = {
  ...
  storageType: process.env.VITE_ASSET_STORAGE_TYPE || process.env.ASSET_STORAGE_TYPE || 'local',
  
  // Cloudflare R2
  r2: {
    accountId: process.env.VITE_R2_ACCOUNT_ID || process.env.R2_ACCOUNT_ID,
    bucketName: process.env.VITE_R2_BUCKET_NAME || process.env.R2_BUCKET_NAME || 'nihongo-assets',
    accessKeyId: process.env.R2_ACCESS_KEY_ID,
    secretAccessKey: process.env.R2_SECRET_ACCESS_KEY,
    publicDomain: process.env.VITE_R2_PUBLIC_DOMAIN || process.env.R2_PUBLIC_DOMAIN,
  },
  
  // AWS S3
  s3: {
    region: process.env.VITE_S3_REGION || process.env.S3_REGION || 'us-east-1',
    bucketName: process.env.VITE_S3_BUCKET_NAME || process.env.S3_BUCKET_NAME || 'nihongo-assets',
    accessKeyId: process.env.S3_ACCESS_KEY_ID,
    secretAccessKey: process.env.S3_SECRET_ACCESS_KEY,
    publicDomain: process.env.VITE_S3_PUBLIC_DOMAIN || process.env.S3_PUBLIC_DOMAIN,
  },
  ...
  supabase: {
    ...
    bucketName: process.env.VITE_SUPABASE_BUCKET_NAME || process.env.SUPABASE_BUCKET_NAME || 'kanji-gifs',
  },
...
class StorageProvider {
  ...
  async uploadAll(files) {
    ...
    for (const file of files) {
      try {
        const url = await this.uploadFile(file.filename, file.content, file.contentType);
        ...
        results.push({ filename: file.filename, url: null, success: false, error: error.message });
      }
    }
    return results;
  }
...
async function generateManifest(files, provider) {
  ...
  for (const file of files) {
    ...
    assets[kanjiId] = {
      ...
      url: config.cdnBaseUrl ? `${config.cdnBaseUrl}/${file.filename}` : publicUrl,
      alternateUrls,
      fileSize: fileStat ? fileStat.size : undefined,
      contentType: getContentType(file.filename),
      uploadedAt: new Date().toISOString(),
      ...(config.cdnBaseUrl && { cdnUrl: `${config.cdnBaseUrl}/${file.filename}` }),
    };
  }
  ...
  return {
    ...
    storageConfig: {
      ...
      ...(config.storageType === 's3' && { bucket: config.s3.bucketName, region: config.s3.region }),
      ...(config.storageType === 'supabase' && { bucket: config.supabase.bucketName }),
    },
    assets,
  };
}
...
async function main() {
  ...
  const generateOnly = args.includes('--generate-manifest') || args.includes('-g');
  
  if (generateOnly) {
    log('Generating manifest only (no upload)', 'info');
  } else {
    log(`Uploading ${fileContents.length} files to ${config.storageType}...`, 'info');
  }
  
  // Upload files and generate manifest
  const manifest = await generateManifest(fileContents, provider);
  
  log(`Generated manifest with ${Object.keys(manifest.assets).length} assets`, 'info');
  ...
  if (config.storageType === 'local') {
    ...
    log('In production, set VITE_ASSET_STORAGE_TYPE to r2, s3, or supabase', 'warn');
  }
