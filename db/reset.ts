 ...
 * Clears all data and reseeds from SQL migration file
...
async function reset() {
  ...
  try {
    ...
    // Seed with migration data from SQL file
    const sqlPath = path.join('db', 'migrations', '0002_seed_vocabulary.sql');
    const migrationVocabulary = parseSqlVocabulary(sqlPath);
    
    console.log('🌱 Seeding from SQL file...');
    console.log(`📦 Total items to seed: ${migrationVocabulary.length}`);
    ...
    console.log(`✅ Successfully seeded ${successCount} vocabulary items`);
    ...
    console.log(`\n✨ Total: ${total.length} vocabulary items`);
